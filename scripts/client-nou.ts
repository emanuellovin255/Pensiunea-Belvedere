/**
 * npm run client-nou -- <nume> --sablon <1|2|3> [--forteaza]
 *
...
function numeSablon(n: number): string {
  return { 1: 'Hero Video', 2: 'Poveste alternantă', 3: 'Galerie editorială' }[n] ?? '—'
}
...
function main() {
  ...
  if (!nume || nume.startsWith('-')) {
    console.error('\n  Utilizare: npm run client-nou -- <nume> --sablon <1|2|3> [--forteaza]\n')
    process.exit(1)
  }
  const sablon = Number(arg('--sablon') ?? '2')
  if (![1, 2, 3].includes(sablon)) {
    console.error('\n  --sablon trebuie să fie 1, 2 sau 3.\n')
    process.exit(1)
  }
