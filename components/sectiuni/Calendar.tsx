        ...
        {celule.map((c, i) => {
          ...
          return (
            <button
              ...
              aria-label={`${c.zi} ${LUNI[luna].toLowerCase()} ${an}`}
  ...
  return (
    <div className="cal">
      <div className="cal-bara">
        <button type="button" className="cal-nav" onClick={() => muta(-1)} disabled={inapoiOprit} aria-label="Luna anterioară">
          <Icon name="arrow" marime="sm" />
        </button>
        <button type="button" className="cal-nav" onClick={() => muta(1)} aria-label="Luna următoare">
          <Icon name="arrow" marime="sm" />
        </button>
      </div>

      <div className="cal-luni">
        <Luna an={ancora.an} luna={ancora.luna} checkIn={checkIn} checkOut={checkOut} minim={minim} onAlege={alege} />
        <Luna
          an={urm.getFullYear()}
          luna={urm.getMonth()}
          checkIn={checkIn}
